...
describe('pre-existing hashes and actual filesystem', () => {
  it('finds no pre-existing files when none given or exist', async () => {
    const expected = {}
    const actual = await findNew('chapter-05/test/bck-0-csv-0', [])
  ...
  it('finds some files when one is given and none exist', async () => {
    ...
    const actual = await findNew('chapter-05/test/bck-0-csv-0', check)
    assert.deepStrictEqual(expected, actual,
      'Expected one file')
  })
  ...
  it('finds nothing needs backup when there is a match', async () => {
    ...
    const expected = {}
    const actual = await findNew('chapter-05/test/bck-1-csv-1', check)
    assert.deepStrictEqual(expected, actual,
      'Expected no files')
  })
  ...
  it('finds mixed matches', async () => {
    ...
    const actual = await findNew('chapter-05/test/bck-4-csv-2', check)
    assert.deepStrictEqual(expected, actual,
      'Expected one file')
  })
})
